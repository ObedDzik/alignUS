...
# MedSAMIBot (projects.medibot.src.models.medsam) was imported here but only
# ever referenced inside a commented-out block below -- dropped so this
# vendored copy doesn't depend on an unrelated project that isn't part of
# this repo.
...
class ProstNFound(nn.Module):
    ...
    def forward(
        ...
        **prompts,
        ...
        #TODO: THIS CODE IS THE ORIGINAL
        # B, C, H, W = image.shape
        # image_feats = self.medsam_model.image_encoder(image)
        # if getattr(self.medsam_model, "adapter", None) is not None:
        #     image_feats = self.medsam_model.adapter(image_feats) # using external adapter
        ...
        if output_mode == "heatmaps":
            ...
            return dict(mask_logits=mask_logits, cls_outputs=cls_outputs, iou=iou, image_feats=image_feats)
            #TODO: This is from Tarek
            # return dict(mask_logits=mask_logits, cls_outputs=cls_outputs, iou=iou,image_feats=image_feats, image_feats_priorneck=image_feats_priorneck)
...
@register_model
def prostnfound_adapter_medsamibot(
    adapter_dim=256,
    prompts=["age", "psa", "approx_psa_density"],
    checkpoint=None,
    upsample=False,
    **kwargs,
    ...
    if upsample:
        ...
        backbone = "medsam_adapter_medsamibot"

    return prostnfound(
        backbone=backbone,
        backbone_kw=dict(adapter_dim=adapter_dim,checkpoint=checkpoint, **kwargs.pop("backbone_kw", {})),
        prompts=prompts,
        **kwargs,
    )
